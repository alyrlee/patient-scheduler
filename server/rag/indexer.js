...
// robust chunker: 1.5k chars with 200 overlap (tweak as needed)
function chunk(text, size = 1500, overlap = 200) {
  ...
  async function upsertPgvector(docs) {
    ...
    // one big transaction for the run
    await client.query('BEGIN');
  
    try {
      for (const d of docs) {
        const parts = chunk(d.text, 1500, 200);
        const ids   = parts.map((_, i) => `${d.id}#${i}`);
        // ⬇️ batch embed the chunks for this doc
        const embs  = await embeddings.embedDocuments(parts); // returns number[][]
  
        for (let i = 0; i < parts.length; i++) {
          const vec = `[${embs[i].join(",")}]`; // pgvector literal
          await client.query(insertSQL, [ids[i], d.dept, `${d.title} (${i+1})`, parts[i], vec]);
        }
        console.log(`upserted ${d.id} (${parts.length} chunks)`);
      }
